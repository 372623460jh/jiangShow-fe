...
;(function (exports) {
    ...
    var Router = exports.$jh.Router = function (routes) {
...
;(function defineMustache(global, factory) {
    ...
    // } else {
    global.$jh.Mustache = {};
    factory(global.$jh.Mustache); // script, wsh, asp
    // }
    ...
    function parseTemplate(template, tags) {
        ...
        var $jhces = [];       // Indices of white$jhce tokens on the current line
        var hasTag = false;    // Is there a {{tag}} on the current line?
        var non$jhce = false;  // Is there a non-$jhce char on the current line?

        // Strips all white$jhce tokens array for the current line
        // if there was a {{#tag}} on it and otherwise only $jhce.
        function strip$jhce() {
            if (hasTag && !non$jhce) {
                while ($jhces.length)
        ...
        while (!scanner.eos()) {
            ...
            if (value) {
                for (var i = 0, valueLength = value.length; i < valueLength; ++i) {
                    ...
                    // Check for white$jhce on the current line.
                    if (chr === '\n')
                        strip$jhce();
                }
            }
...
;(function (global, undefined) {
    ...
    Page.prototype = {
        ...
        destroy: function () {
            //移除元素
            data.container.removeChild(this.dom);
        },
        // 深拷贝对象
